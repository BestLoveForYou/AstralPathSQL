...
import static org.astralpathsql.server.MainServer.all_Connect;
...
public class Filer {
    public static void createInFirst() {
        try {
            ...
            File file2 = new File("." + File.separator + "apsql" + File.separator + "config" + File.separator + "info.properties");			// 定义文件路径
            if (!file2.getParentFile().exists()) { 							// 父路径不存在
                file2.getParentFile().mkdirs(); 							// 创建父路径
            }
            if (file2.exists()) {											// 文件存在

            } else { 													// 文件不存在
                System.out.println(file2.createNewFile()); 					// 创建新的文件
                ProRead.write();
            }
